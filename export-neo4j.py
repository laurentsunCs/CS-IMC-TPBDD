...
with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password) as conn:
    ...
    while True:
        ...
        for row in rows:
            ...
            film_node = Node("Film", idFilm=row[0], primaryTitle=row[1], startYear=row[2])
            importData.append(film_node)
            i += 1
    ...
    while True:
        ...
        for row in rows:
            ...
            artist_node = Node("Artist", idArtist=row[0], primaryName=row[1], birthYear=row[2])
            importData.append(artist_node)
            i += 1
    ...
    while True:
        ...
        try:
            for cat in importData:
                ...
                create_relationships(graph.auto(), importData[cat], rel_type=cat.replace(' ', '_'))
            exportedCount += len(rows)
